...
public class cambiarClave {
	...
	public class CambiarClave extends HttpServlet {
	    ...
	    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
	        ...
	        String clave3=request.getParameter("clave_tarjeta3");
	        String cliente=request.getParameter("cliente");
	        
	        user=buscar.consultar_claves(clave1, clave2);
	        
	        if (user!= null){
	        resultado=modifica.modificarClave(cliente, clave3);
	        if (resultado){
	            response.sendRedirect("cambiar_clave.jsp?ok=ok");
	        }else{
	            response.sendRedirect("cambiar_clave.jsp?e=0");
	        }
	        }else{
	        response.sendRedirect("cambiar_clave.jsp?e=1");
	        }
