/* --------------------------------------------------
    Pertho Admin Panel js configuration file     v1.0
-----------------------------------------------------
:: Global
:: Charts
:: Forms
	...
	prth_charts = {
		ds_plot1: function(){
			...
			dsPlot1 = $.jqplot('ds_plot1', [dsPlot1_data], {
				...
				seriesColors: ["#058DC7"],
				title: 'Total de Ingresos del mes de Junio del 2012',
...
// ----- Forms ----- //
	...
	prth_clearForm = {
		init: function() {
			...
			$('.clear_form').click(function() {
				var clear_btn = $(this);
				$.fancybox(
					'<div class="tac"><p class="sepH_b">¿Desea limpiar los datos del formulario?</p><a href="javascript:void(0)" id="clear_yes" class="gh_button small">Si</a><a href="javascript:void(0)" id="clear_no" class="gh_button small danger">No</a></div>',
					{
						'transitionIn'		: 'elastic',
		...
		init: function() {
			form_validator = $("form.validate").validate({
				...
				invalidHandler: function(form, validator) {
					...
					$('body').showMessage({
						thisMessage			: ['El Formulario Contiene <span class="errorsNb">'+errors+'</span> errores, por favor verificar.'],
						className			: 'fail',
						autoClose			: false
					});
				},
				...
				messages: {
					tipo_tarjeta: "Porfavor seleccione su tipo de tarjeta",
					codcaptcha:{
						required: "Porfavor ingrese el codigo que se muestra en la Imagen",
						minlength: "El codigo debe contener 6 digitos"
						},
					num_tarjeta:{
						required:"Porfavor Ingrese su Numero de tarjeta",
						number:"El numero de tarjeta solo deben ser digitos numericos",
						minlength:"El numero de tarjeta debe contener 16 digitos"
					},
					clave_tarjeta:{
						required:"Porfavor Ingrese su Clave de 6 Digitos",
						minlength:"La clave debe contener 6 digitos"
						},
					password: {
				...
				submitHandler: function(form) {
					...
					$.post('aaaa', post, function(data) {
						$('body').showMessage({
							thisMessage			: ['Datos Enviados:<br/>'+data],
							className			: 'success',
							autoClose			: true,
							delayTime			: 10000
						});
	...
	//* select validation
	prth_select_validation = {
		init: function() {

			select_validator = $("form.select_validation").validate({
				highlight: function(element) {
					$(element).closest('div.elVal').addClass("form-field error");
					$(element).closest('div.elVal').find('select, .chzn-single').css("borderColor","#c00000");
				},
				unhighlight: function(element) {
					$(element).closest('div.elVal').removeClass("form-field error");
                    $(element).closest('div.elVal').find('select, .chzn-single').css("borderColor","");
				},
				invalidHandler: function(form, validator) {
					//scroll to top
					$('html,body').animate({ scrollTop: $('form.select_validation').offset().top - 34 }, 'slow');
				},
				rules: {
					simple_select: { required: true },
					multiple_select: { required: true,  minlength:2 },
					multiple_select_range: { required: true,  rangelength:[1,2] },
					select_enhanced: { required: true }
				},
				errorPlacement: function(error, element) {
					error.appendTo( element.closest("div.elVal") );
				},
				//submit form
				submitHandler: function(form) {
					var post = $("form.select_validation").serializeObject();
	...
	prth_mask_input = {
		init: function() {
			...
			$("#num_cuenta").inputmask("999-99999999-9-99",{placeholder:" "});
		}
	};
	...
	prth_limiter = {
		init: function(){
			...
			//$('#txtarea_limit_chars').after('<span>You have <span id="counter_chars"></span> characters left.</span>');
			//$('#txtarea_limit_words').after('<span>You have <span id="counter_words"></span> words left.</span>');
			//$('#txtarea_limit_chars').simplyCountable({
			//	counter: '#counter_chars',
			//	maxCount: 100,
			//	countDirection: 'down',
			//	strictMax: true,
			//	safeClass: 'lbl ok_bg'
			//});
			//$('#txtarea_limit_words').simplyCountable({
			//	counter: '#counter_words',
			//	maxCount: 20,
			//	countType: 'words',
			//	countDirection: 'down',
			//	strictMax: true,
			//	safeClass: 'lbl info_bg'
			//});
		}
	};
	...
	prth_ios_checkboxes = {
		init: function(){
			...
			//$('#on_off').iphoneStyle();
			//$('#on_off_on').iphoneStyle();
			//$('#disabled').iphoneStyle();
			//$('#css_sized_container').iphoneStyle({
			//	resizeContainer: false,
			//	resizeHandle: false
			//});
			//$('#long_tiny').iphoneStyle({
	...
	prth_contacts_list = {
		init: function() {
			$('#contactList').listnav({
				...
				includeOther: false,
				flagDisabled: true,
				showCounts: true //changed to default tooltips, not visible on touch devices
			});
		}
	};
	...
	prth_dialogs = {
		init: function(){
			...
			countdown = setInterval(function(){
				if(count == 10){
					$('body').showMessage({
						thisMessage		: ['Error al realizar operacion: porfavor verifique que los datos ingresados sean correctos y vuelva a intentarlo.'],
						className		: 'fail',
						autoClose		: true,
	...
	prth_dialogs1 = {
		init: function(){
			...
			countdown = setInterval(function(){
				if(count == 10){
					$('body').showMessage({
						thisMessage		: ['Operacion realizada correctamente: felicidades la operacion solicitada se ha realizado correctamente.'],
						className		: 'success',
						autoClose		: true,
						delayTime		: 10000 //in miliseconds
					});
			...
			$('#validate_wizard').validate({
				errorPlacement: function(error, element) {
					...
					'clave_tarjeta31'	: {
						required:true,
						minlength: 6,
						equalTo: "#clave_tarjeta2"
						},
					'clave_caduca'	: 'required',
					'captcha'		: {
						required:true,
						minlength: 6
						},
					...
					'codcaptcha'	: { 
										required:  'Porfavor ingrese el codigo mostrado en la imagen!', 
										minlength: 'El codigo debe contener 6 digitos!'
										},
					'num_tarjeta'	: { 
										required:  'Porfavor ingrese su numero de tarjeta!', 
										number: 'El numero de tarjeta solo debe contener digitos!',
										minlength: 'El numero de tarjeta debe contener 16 digitos!'
										},
					'num_cuenta'	: { 
										required:  'Porfavor ingrese un numero de cuenta!', 
										minlength: 'El numero de cuenta debe contener 17 digitos!'
										},
					'txtdni'	: { 
										required:  'Porfavor ingrese el numero de DNI del titular!', 
										number: 'El numero de DNI solo debe contener digitos!',
										minlength: 'El numero de DNI debe contener 8 digitos!'
										},
					
					'tipo_tarjeta'	: { required:  'Porfavor seleccione su tipo de tarjeta!' },
					
					'clave_tarjeta1'	: { 
										required:  'Porfavor Ingrese su clave de 4 digitos!', 
										minlength: 'La clave debe contener 4 digitos!'
										},
					'clave_tarjeta2'	: { 
										required:  'Porfavor Ingrese su clave de 6 digitos!', 
										minlength: 'La clave debe contener 6 digitos!'
					...
					'clave_tarjeta31'	: { 
										required:  'Porfavor Vuelva a ingresar su clave de 6 digitos!', 
										minlength: 'La clave debe contener 6 digitos!',
										equalTo: 'Las claves no coinciden!'
										},
					'clave_caduca'	: { required:  'Seleccione una opcion!' },
					
					'captcha'	: { 
										required:  'Porfavor ingrese el codigo que se muestra en la imagen!', 
										minlength: 'El codigo debe contener 6 digitos!'
										},
					'tipo_servicio'	: { required:  'Porfavor seleccione un servicio!' }
				},
				ignore				: ':hidden'
			});
		},
